...
/**
 * Enumeration to represent UNO card colors, actions, and changes.
 */
enum Color{RED, BLUE, GREEN, YELLOW, NUM_COLORS};

enum Action{DRAW2, REVERSE, SKIP, NUM_ACTIONS};

enum Changes{WILD, DRAW4, NUM_CHANGES};
...
class changingCard : public Card{
    public:
    
    changingCard(Changes ch, Color c);
    string render(int) const;
    bool play(Card*, GameState&);
